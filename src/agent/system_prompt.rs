...
//! It is deliberately short and states only rules the tools can't express
//! in their own descriptions — no aspirational rules the harness can't
//! enforce. It must also be byte-stable for the session: it precedes the
//! conversation cache breakpoint, so volatile state (todos, cwd) rides the
//! message stream instead of living here.

use crate::agent::state::AgentState;

pub const BASE_PROMPT: &str = r#"You are an interactive coding agent running inside myharness, a terminal-based harness. You help the user with software engineering tasks in their workspace. Each tool's contract is stated in its own description; the rules here sit on top of those.
...
pub fn build_system(state: &AgentState, subagent: bool) -> String {
    ...
    // The prompt must stay byte-stable for the whole session: it sits before
    // the conversation cache breakpoint, so any change re-bills the entire
    // history as a cache write. Volatile state (todos, cwd changes) rides the
    // message stream instead — todo_write echoes the list in its tool result,
    // bash reports a changed cwd in its result, and compaction folds the todo
    // list into the handoff summary.
    let mut s = format!(
        "{base}\n\n# Environment\n- OS: {}\n- Workspace root: {}",
        std::env::consts::OS,
...
mod tests {
    ...
    /// The prompt-cache prefix runs tools → system → messages, so the system
    /// prompt must be byte-identical for the whole session: volatile state
    /// (todos, cwd) must not leak into it.
    #[test]
    fn system_prompt_is_byte_stable_across_todo_and_cwd_changes() {
        ...
        assert!(!second.contains("task list"), "todos must not ride the system prompt");
        assert!(!second.contains("subdir"), "live cwd must not ride the system prompt");
    }
}
